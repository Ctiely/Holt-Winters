...
@author: clytie, Renmin University of China
...
class HoltWinters(object):
    ...
    def Compute_InitValues(self):
        ...
        if self.mtype == 'additive':
            rawSeason  = initSeries - initSeries.rolling(p, p, center=True).mean()
    ...
    def Mape(self):
        """
        Can't use this evaluator if the true data has 0 values.
        """
    ...
    def Mse(self):
        if self.pred is None:
            raise Exception("You must run function PredictValues before" +
                            "you want to compute mse of test set!")
        y_true = self.ts_test
        y_pred = self.pred
        mse = HoltWinters.Compute_Mse(y_true, y_pred)
        self.mse = mse
        return(mse)
    ...
    def Rmse(self):
        if self.pred is None:
            raise Exception("You must run function PredictValues before" +
                            "you want to compute rmse of test set!")
        y_true = self.ts_test
        y_pred = self.pred
        rmse = HoltWinters.Compute_Rmse(y_true, y_pred)
        self.rmse = rmse
        return(rmse)
    ...
    def GridSearch(self, alphas=None, betas=None, gammas=None, err_type="MSE"):
        """
        @params:
            ...
            - gammas  [list or numpy]:  user-specified season forgetting factor 
                                      (np.arange(0.1, 1, 0.1) if None)
            - err_type[string]:  evaluation type: contain MSE, MAPE, RMSE
        """
        ...
        for alpha, beta, gamma in grids:
            ...
            if err_type == "MSE":
                err = HoltWinters.Compute_Mse(self.ts_test, y_pred)
            elif err_type == "MAPE":
                err = HoltWinters.Compute_Mape(self.ts_test, y_pred)
            elif err_type == "RMSE":
                err = HoltWinters.Compute_Rmse(self.ts_test, y_pred)
            else:
                raise Exception("If you want to use other evaluator, you must identify" + 
                                "it before using.")
...
if __name__ == '__main__':
    data1 = pd.read_csv('/Users/clytie/Documents/研究生/Mpython/rent_data_402.csv'.decode('utf-8'))
    number = data1['number']
    days = data1[['year', 'month', 'day']]\
                .apply(lambda dte: '{}/{}/{}'.format(dte[0], dte[1], dte[2]),
                       axis=1)
    number = data1.groupby(['year', 'month', 'day'])['number'].sum()
    number.index = days.unique()
    number = number.astype('float')        
    
    HWmodel = HoltWinters(number, 365, 14)
    HWmodel.GridSearch()
    
    plt.plot(np.array(HWmodel.ts_test))
    plt.plot(HWmodel.best_pred)
